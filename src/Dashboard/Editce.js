...
const EditCe = () => {
    ...
    useEffect(() => {
            ...
            fetch(`${urls.api}issue/index.php`, { body: fd, method: 'POST' })
                ...
                .then(res => {
                    if (res.msg === "1") {
                        ...
                        loadAgentinfos(res.data.agent.agent_id);        
                        }               
                    } else if (res.msg === "404") {
                        alert(res.data);
                        localStorage.removeItem('username');
        ...
        function loadAgentinfos(agentid) {
            let fd = new FormData();
            fd.append('username', localStorage.getItem('username'));
            fd.append('usertoken', localStorage.getItem('usertoken'));
                    ...
                    <form onSubmit={(e) => { updateuser(e) }}>
                        ...
                        <div className="naf-ams-tablediv" style={{ marginTop: '10px' }}>
                            <div className="naf-asm-table-row">
                                ...
                                <div className="naf-ams-table-cell table-lable">{uaccess?'Agent' : ''}</div>
                                <div className="naf-ams-table-cell table-inputs">
                                   <select value={user_agent} className="arnewinput" onChange={getagetinfos}>
                                        <option value="">-select-</option>
